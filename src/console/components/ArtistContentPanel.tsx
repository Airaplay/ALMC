...
import { Disc3, Music, Plus, Search, Video, X } from 'lucide-react';
...
export function ArtistContentPanel({ artist, onClose }: ArtistContentPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4">
      <div
        className={`${consoleTheme.card} flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden sm:rounded-2xl`}
      >
        <div className="flex items-start justify-between gap-3 border-b border-border px-5 py-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h3 className="truncate text-lg font-bold text-foreground">{artist.stage_name}</h3>
              <p className="text-sm text-muted-foreground">
                {total} content item{total === 1 ? '' : 's'}
              </p>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            {hasPermission('content.upload') && (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-[#3ba208] px-3 py-2 text-xs font-bold text-white hover:bg-[#3ba208]/90"
              >
