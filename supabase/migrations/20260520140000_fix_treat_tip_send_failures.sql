/*
  # Fix "Failed to send tip" for Treat tips

  Common causes:
  1) treat_tips had DELETE policies but no INSERT/SELECT for authenticated senders
     (RLS blocks the client insert with a generic API error).
  2) process_treat_tip_transactions / trigger_update_treat_wallet on production may
     predate promo_balance support — users with only signup promo treats pass the
     app balance check but fail DB validation (balance-only check).

  This migration is idempotent: policies are created only if missing; functions are
  replaced with the promo-aware versions from 20260515284000.
*/

-- ---------------------------------------------------------------------------
-- 1) RLS: allow authenticated users to send and read their own tips
-- ---------------------------------------------------------------------------
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'treat_tips'
      AND policyname = 'Users can insert tips they send'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'treat_tips'
      AND policyname = 'Users can view tips they sent or received'
  ) THEN
...
-- ---------------------------------------------------------------------------
-- 2) Wallet trigger: spend promo_balance first (re-apply latest logic)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Tip processor: validate balance + promo_balance (re-apply latest logic)
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.process_treat_tip_transactions() IS
  'Creates tip_sent / tip_received treat_transactions rows. Validates sender (balance + promo_balance) >= tip amount.';
