/*
  # Exclude Treat purchases from Net USD Revenue (usable platform revenue)

  platform_revenue_net_usd = AdMob usable net + External net only.
  Treat purchases remain in gross revenue and have their own breakdown card.
*/
...
BEGIN
  ...
  -- Usable platform revenue: AdMob + External only (Treat purchases excluded)
...
COMMENT ON FUNCTION public.admin_get_analytics_overview_totals() IS
  'Admin overview totals. platform_revenue_net_usd = AdMob usable net + External net (Treat purchases excluded). platform_revenue_gross_usd includes Treat. platform_share_revenue_usd = platform-retained share only.';
