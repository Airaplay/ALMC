/*
  # Add platform-share-only revenue to Analytics Overview

  Platform Revenue (platform retained/share only):
  - AdMob: usable net residual after active ad_safety_caps artist/listener split
  - Treat purchases: full completed IAP USD (platform-collected)
  - External revenue: platform_retained from distributions, else net - distributable
*/
...
COMMENT ON FUNCTION public.admin_get_analytics_overview_totals() IS
  'Admin overview totals. platform_share_revenue_usd = platform-retained share only (AdMob caps residual + Treat IAP + External platform retained). platform_revenue_gross_usd/net_usd = all incoming revenue sources.';
